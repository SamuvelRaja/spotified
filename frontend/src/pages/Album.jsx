import { useGetAlbumQuery } from "../redux/slice/shazamapi"
...
import { setSong,setImage } from "../redux/features/playerSlice";
import {Loadertracks} from "/src/components/Loader"

const Album = () => {
    const dispatch=useDispatch()
    const{id}=useParams()
    const{data, isFetching, error}=useGetAlbumQuery(id)
  ...
  function playSong(i){
    ...
    dispatch(setImage(data?.images[2]?.url))
  }
          ...
          <div className="px-6 pt-20 pb-[60px] rounded-t-md" style={{background:primaryColor.primary1}}>
            <div className="flex flex-col md:flex-row gap-4 md:items-end md:h-[232px]">
              ...
              <div className="flex flex-col drop-shadow-sm">
                ...
                <h1 className="mt-2 drop-shadow-sm pr-6 text-[32px] md:text-[48px] xl:text-[72px] w-full max-w-[65vw] 2xl:text-[96px] font-spotifyTitle whitespace-nowrap truncate">{data?.name}</h1>
                ...
                <div className="mt-4 flex items-center">
                  ...
                  <p className=" text-[14px] font-thin">
                    <b className="font-semibold drop-shadow-sm"> Spotify </b>  . {data?.tracks?.total} songs
                  </p>
